...
import (
	"errors"
	"fmt"
	appUnload "jrtc-ctl/cmd/app/unload"
	"jrtc-ctl/cmd/clicommon"
	jbpfUnload "jrtc-ctl/cmd/jbpf/unload"
	"jrtc-ctl/common"
...
// Command Unload a jrt-controller application package
func Command(opts *common.GeneralOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "unload",
		Short: "Unload a jrt-controller application package",
		Long:  "Unload a jrt-controller application package comprised of apps and codelet sets from a jrt-controller environment.",
...
func run(cmd *cobra.Command, opts *runOptions) error {
	...
	jbpfClients := make(map[uint8]*jbpf.Client)
	...
	for _, app := range cfg.Apps {
		...
		g.Go(func() error {
			client, ok := jrtcClients[fmt.Sprintf("%s:%d", a.IP, a.Port)]
			if !ok {
				return errors.New("missing jrt-controller Controller client")
			}
			return appUnload.App(logger, client, app.Name)
		})
	}
	...
	for _, jbpfCodeletSet := range cfg.JBPF.JBPFCodelets {
		...
		g.Go(func() error {
			return jbpfClients[j.Device].UnloadCodeletSet(codeletCfg.ID)
		})
	}
	...
	for _, jbpfCodeletSet := range cfg.JBPF.JBPFCodelets {
		...
		for _, currentDecoder := range decoders {
			dec := currentDecoder
			g.Go(func() error {
				return jbpfUnload.SchemasDecoder(dec, codeletCfg, cfg.Name)
			})
		}
	}
