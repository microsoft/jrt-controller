...
char *expand_env_vars(const char *input) {
    ...
    while (regexec(&regex, expanded, 2, match, 0) == 0) {
        ...
        if (!env_value) env_value = "";  // Default to empty string if not found
        ...
        snprintf(new_expanded, new_len + 1, "%.*s%s%s", (int)prefix_len, expanded, env_value, expanded + match[0].rm_eo);
