...
public:
    ...
    JrtcApp(struct jrtc_app_env* env_ctx,
            JrtcAppCfg_t* app_cfg,
            JrtcAppHandler app_handler,
            void* app_state);
    ...
    // @return Stream ID
    jrtc_router_stream_id_t get_stream(int stream_idx);
    
    // Retrieves the channel context for a given stream index
    // @param stream_idx - Index of the stream
    // @return Channel context
    dapp_channel_ctx_t get_chan_ctx(int stream_idx);
